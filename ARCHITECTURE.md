...
- **Manual face data (participant detail)**: **`ParticipantDetailFragment`** lists stored **`participant_embeddings`** (with photo preview when a path exists), **Remove face data** clears all embeddings for every protocol row linked to the same **`identity_registry`** as the opened row (or that row only), syncs legacy **`race_participant_hashes.embedding`**, clears the registry’s stored face vector and thumbnail when linked, and refreshes **`protocol.xml`** per affected race—scan codes, display names, finish detections, and protocol finish times are not removed. **Delete device identity** (with confirmation) deletes the **`identity_registry`** row and sets **`identityRegistryId`** to null on linked protocol rows; race protocol entries and finish rows stay; **`protocol.xml`** is refreshed.
...
3. **Start-photo face size filter** (same calibration as finish photos): unconditional hard floor `SMALL_FACE_HARD_FLOOR_PX2 = 3 000 px²`; conditional relative + absolute filter active when the largest detected face ≥ `SMALL_FACE_MIN_AREA_PX2 = 10 000 px²`, with effective per-photo cutoff = `max(10 000, maxFaceArea / 3)`. Rationale: crowded pre-start photos (e.g., 15 detected faces, max 199 k px²) generate many small-face embeddings that are too low quality to match reliably and cause spurious finish-line identifications. **Peripheral filter** (`FaceGeometryFilters`): after size filtering, faces with `|cx − 0.5| > 0.20` are discarded, **except** a frontal face (`width/height ≥ 0.65`) in a **peer group** of 2–4 size-filtered faces with `|cx − 0.5| ≤ 0.30`. Extreme edges (`|cx − 0.5| > 0.30`), profiles, lone off-centre faces, and crowded frames (5+ size-filtered faces) still use the strict 0.20 band. Prevents profile/back-of-head bystanders at the frame edge from seeding the pool, without dropping the outer people in a 2–3 person group shot.
...
2. Per face: two pre-embedding filters run before the crop/embed step.
   - **Edge-of-frame filter (two parts)**:
     ...
     - *Part B — Frontal-edge*: if `width/height ≥ 0.65` (frontal face) and `|cx − 0.5| > 0.20` and raw area ≥ 15 000 px², the face is skipped when any other face in the same photo is strictly more central — **unless** the photo is a peer group of 2–4 size-filtered faces and this face is still within `|cx − 0.5| ≤ 0.30`. The area floor protects small, distant approaching runners (area typically < 15 k px²); nearby bystanders at the extreme edge are still dropped. Calibrated on race cc7a5378; peer-group exception on 2026-08-29 group photos.
...
**`RaceRepository`** is the app-facing façade (same public methods as before); focused pieces live in **`internal`** types in the same package so they can depend on other **`internal`** helpers without leaking those types in public signatures: race lifecycle + `race.xml` (**`RaceLifecycleStore`**), per-race embedding read model for matching (**`RaceParticipantEmbeddingReader`**), event-photo delete + DB/XML side effects (**`RaceEventPhotoDeletionService`**), participant ↔ races history rows (**`ParticipantRaceHistoryReader`**), participant-detail face strip + registry delete (**`ParticipantFaceDataMutations`**), plus the existing writers/sync/merge types listed in **`CODE_EXPLORER.md`**.
...
- **`RacePhotoProcessor`** emits structured lines to **`VirtualVolunteerApp.appendPipelineLog`** (in-memory ring buffer **`VirtualVolunteerApp.PIPELINE_DEBUG_MAX_LINES`** ≈ 8k lines, also Logcat): start/finish phases, face counts, **multi-embedding** nearest/match cosines, duplicate skips, **`recordFinishDetection`** results, **`embeddingAppended`**, merge summaries from **`RaceRepository`** when duplicate scans collapse two rows.
- **`FinishPhotoAnalysisQueue`** (CameraX path): pipeline lines per enqueue (`FINISH_QUEUE_ENQUEUED` / skip empty-missing), per completed job (`FINISH_QUEUE_OK` / `FINISH_QUEUE_FAIL` with **`FinishProcessResult.debugSummaryLine`**), and **full** per-file **`FinishProcessResult.logText`** in Logcat under tag **`FinishPhotoAnalysisQueue`** (in-app buffer is capped at **`PIPELINE_DEBUG_MAX_LINES`**; very long sessions may still drop the oldest lines).
- **Offline import** (`RaceDetailPhotoBulkImporter`) logs each finish URI with **`IMPORT_FINISH_OK`** + summary line; full **`logText`** in Logcat tag **`RaceDetail`**.
- **Reprocess from disk**: **`RaceRepository.executeFullDiskPhotoReprocess`** accepts optional **`progressLog`** (`REPROCESS_BEGIN`, per-file **`REPROCESS_START_FILE`** / **`REPROCESS_FINISH_FILE`**, **`REPROCESS_END`**); **`RacePhotoProcessor.reprocessRaceFromStoredEventPhotos`** wires that to the pipeline and adds **`REPROCESS_*_DETAIL`** lines plus Logcat dumps of finish **`logText`**. **`RaceReprocessResult`** includes **`startIngestFailures`** / **`finishIngestFailures`** when **`Result`**-based ingest throws.
- Finish **ingest** returns **`FinishProcessResult`** with **`logText`**, **`decodeSucceeded`**, **`detectedFaceCount`**, plus **`finishOutcomeLabel`** / **`debugSummaryLine`** for one-liners. Finish folder import ends with a **`importFinishPhotos SUMMARY`** pipeline line and a Toast with counts (processed, errors, no-face photos, decode failures, sum of faces, new rows).
...
- Face geometry filters: `domain/face/FaceGeometryFilters.kt`
